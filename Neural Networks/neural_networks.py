...
class relu:
    ...
    def backward(self, X, grad):
        ...
        backward_output = derivative * grad
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        derivative = 1 - np.multiply(np.tanh(X),np.tanh(X))
        backward_output = np.multiply(derivative, grad) 
        return backward_output
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha > 0.0:
                    ...
                    #################################################################################
                    momentum[module_name + '_' + key]=np.multiply(_alpha,momentum[module_name + '_' + key]) - np.multiply(_learning_rate,g)
                    ...
                    #################################################################################
                    module.params[key]=module.params[key] - np.multiply(_learning_rate,g)
                    #raise NotImplementedError("Not Implemented function: miniBatchGradientDescent")
...
def main(main_params, optimization_type="minibatch_sgd"):
    ...
    for t in range(num_epoch):
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            grad_a1 = model['nonlinear1'].backward(h1, grad_h1)       ######################################################################################
